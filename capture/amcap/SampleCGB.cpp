...
HRESULT ISampleCaptureGraphBuilder::ConnectFilters(
    IBaseFilter* pSrc,
    const GUID* pCategory,
    const GUID* pType,
    IBaseFilter* pDst)
{
    ...
    if (pCategory)
        hr = graphBuilder2_->FindPin(pSrc, PINDIR_OUTPUT, pCategory, pType, true, 0, &pOut);
    else
    ...
    // Find the input pin on the second filter
    IPin* pIn = 0;
    hr = GetUnconnectedPin(pDst, PINDIR_INPUT, &pIn);
    if (FAILED(hr))
    {
        return hr;
    }

    hr = graph_->ConnectDirect(pOut, pIn, NULL);
...
HRESULT ISampleCaptureGraphBuilder::GetUnconnectedPin(
    IBaseFilter* pFilter,
    PIN_DIRECTION PinDir,
    IPin** ppPin)
{
