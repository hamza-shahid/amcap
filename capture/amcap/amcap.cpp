...
// {FDF4DD4D-0B62-44BF-B8BE-25668B8E5CE1}
DEFINE_GUID(CLSID_PrintAnalysis,
    0xfdf4dd4d, 0xb62, 0x44bf, 0xb8, 0xbe, 0x25, 0x66, 0x8b, 0x8e, 0x5c, 0xe1);

// {E5270E8A-6A30-4D02-A0FC-5E5D8134578C}
DEFINE_GUID(CLSID_PrintAnalysisPropertyPage,
    0xe5270e8a, 0x6a30, 0x4d02, 0xa0, 0xfc, 0x5e, 0x5d, 0x81, 0x34, 0x57, 0x8c);
...
struct _capstuff
{
    ...
    HMODULE hEZrgbModule;
    HMODULE hPrintAnalysisModule;
    DWORD dwPrintAnalysisPropPageRegister;
    ...
    int iEZrgbDialogsPos;
    int iPrintAnalysisDlgPos;
    long lDroppedBase;
    long lNotBase;
    BOOL fPreviewFaked;
    CCrossbar *pCrossbar;
...
HRESULT SaveGraphFile(IGraphBuilder* pGraph, LPCWSTR wszPath)
{
    CheckPointer(pGraph, E_INVALIDARG);
    CheckPointer(wszPath, E_POINTER);
    CComPtr<IPersistStream> pPersist;
    HRESULT hr = pGraph->QueryInterface(__uuidof(IPersistStream), (void**)&pPersist);
    if (SUCCEEDED(hr))
    {
...
BOOL BuildPreviewGraphYUV();
BOOL BuildPreviewGraphRGB();
...
void EnableFilterMenu(BOOL bEnable);
...
#ifdef UNICODE
HRESULT LoadFilterFromFile(LPCWSTR pchDllName, HMODULE &hModule, REFCLSID clsid, IBaseFilter **ppFilter)
#else
HRESULT LoadFilterFromFile(LPCSTR pchDllName, HMODULE& hModule, REFCLSID clsid, IBaseFilter** ppFilter)
#endif
{
#ifdef UNICODE
    std::wstring strPrintAnalysisPath = GetExecutableFolderPath() + L"\\" + pchDllName;
    hModule = LoadLibrary(strPrintAnalysisPath.c_str());

#else
    std::string strPrintAnalysisPath = GetExecutableFolderPath() + "\\" + pchDllName;
    hModule = LoadLibrary(strPrintAnalysisPath.c_str());
#endif
    
    if (!hModule)
    {
        ErrMsg(TEXT("Error: filter not found (%s)"), strPrintAnalysisPath.c_str());
        return E_FAIL;
    }
    
    typedef HRESULT(WINAPI* LPFNDLLGETCLASSOBJECT)(REFCLSID, REFIID, LPVOID*);
    LPFNDLLGETCLASSOBJECT pDllGetClassObject;

    pDllGetClassObject = (LPFNDLLGETCLASSOBJECT)GetProcAddress(hModule, "DllGetClassObject");
    if (!pDllGetClassObject) 
    {
        ErrMsg(TEXT("Error: DllGetClassObject not found."));
        return E_FAIL;
    }
    ...
    if (FAILED(hr))
    {
        ErrMsg(TEXT("Error: Unable to find class factory interface for: %s"), pchDllName);
        return E_FAIL;
    }

    hr = pClassFactory->CreateInstance(NULL, IID_IBaseFilter, (void**)ppFilter);
    if (FAILED(hr))
    {
        ErrMsg(TEXT("Error: Unable to create print analysis filter."));
        return E_FAIL;
    }
...
HRESULT ShowPropertyPages(HMODULE hModule, REFCLSID clsid, IBaseFilter *pFilter)
{
    typedef HRESULT(WINAPI* LPFNDLLGETCLASSOBJECT)(REFCLSID, REFIID, LPVOID*);
    LPFNDLLGETCLASSOBJECT pDllGetClassObject;
    ...
    if (FAILED(hr))
    {
        ErrMsg(TEXT("Error: Unable to find class factory interface for the print analysis filter."));
        return E_FAIL;
    }
    ...
    hr = CoRegisterClassObject(
        ...
        &dwPrintAnalysisPropPageRegister);
    ...
    hr = CoRevokeClassObject(dwPrintAnalysisPropPageRegister);
...
BOOL AppInit(HINSTANCE hInst, HINSTANCE hPrev, int sw)
{
    ...
    gcap.hEZrgbModule = NULL;
    gcap.hPrintAnalysisModule = NULL;
    ...
    *szAudioDisplayName = *szVideoDisplayName = 0; // null terminate
    ...
    gcap.pMjpegDec = NULL;
    gcap.pEZrgb = NULL;
    gcap.pPrintAnalysis = NULL;
    gcap.pSelectedFilter = NULL;

    HRESULT hr = LoadFilterFromFile(TEXT("EZRGB24.dll"), gcap.hEZrgbModule, CLSID_EZrgb24, &gcap.pEZrgb);

    //HRESULT hr = CoCreateInstance(CLSID_EZrgb24, NULL, CLSCTX_INPROC_SERVER,
      //  IID_IBaseFilter, (void**)&gcap.pEZrgb);
    if (hr != NOERROR)
    {
        ErrMsg(TEXT("Error: EZrgb24 filter not found"));
    }
    else
    {
        gcap.pSelectedFilter = gcap.pEZrgb;
    }

    hr = LoadFilterFromFile(TEXT("PrintAnalysisFilter.dll"), gcap.hPrintAnalysisModule, CLSID_PrintAnalysis, &gcap.pPrintAnalysis);

    //hr = CoCreateInstance(CLSID_PrintAnalysis, NULL, CLSCTX_INPROC_SERVER,
        //IID_IBaseFilter, (void**)&gcap.pPrintAnalysis);
    if (hr != NOERROR)
    {
        ErrMsg(TEXT("Error: Print Analysis filter not found"));
    }
    else
    {
        gcap.pSelectedFilter = gcap.pPrintAnalysis;
    }
...
void TearDownGraph()
{
    ...
    //SAFE_RELEASE(gcap.pEZrgb);
    //SAFE_RELEASE(gcap.pPrintAnalysis);
    SAFE_RELEASE(gcap.pMjpegDec);
    SAFE_RELEASE(gcap.pRender);
...
BOOL InitCapFilters()
{
   ...
   hr = gcap.pFg->AddFilter(gcap.pVCap, gcap.wachFriendlyName);
    ...
    /*
    WCHAR tempname[] = L"My Video Analyzer Filter";

    StringCchCopyW(gcap.xfrmFriendlyName, sizeof(tempname) / sizeof(tempname[0]), tempname);
    hr = gcap.pFg->AddFilter(gcap.pVXfrm, gcap.xfrmFriendlyName);
    if (hr != NOERROR)
    {
        ErrMsg(TEXT("Error %x: Cannot add Transform to filtergraph"), hr);
        goto InitCapFiltersFail;
    }
    */
...
BOOL BuildPreviewGraphRGB()
{
    ...
    if (gcap.pSelectedFilter == gcap.pEZrgb)
        psFilterName = L"EZrgb24";
    else
        psFilterName = L"Print Analysis";

    // create add the mjpeg decompressor to the graph
    hr = CoCreateInstance(CLSID_MjpegDec, NULL, CLSCTX_INPROC_SERVER,
        IID_IBaseFilter, (void**)&gcap.pMjpegDec);
    if (hr != NOERROR)
    {
        ErrMsg(TEXT("Error creating mjpeg decompressor filter"));
        return FALSE;
    }

    hr = gcap.pFg->AddFilter(gcap.pMjpegDec, L"MJPEG Decompressor");
    if (hr != NOERROR)
    {
        ErrMsg(TEXT("Error %x: Cannot add mjpeg decompressor to filtergraph"), hr);
        return FALSE;
    }

    if (gcap.pSelectedFilter)
    {
        // add the video processing filter to the graph
        hr = gcap.pFg->AddFilter(gcap.pSelectedFilter, psFilterName);
        if (hr != NOERROR)
        {
            ErrMsg(TEXT("Error %x: Cannot add %s to filtergraph"), hr, psFilterName);
            return FALSE;
        }
    }

    // create add the video renderer to the graph
    hr = CoCreateInstance(CLSID_VideoRenderer, NULL, CLSCTX_INPROC_SERVER,
        IID_IBaseFilter, (void**)&gcap.pRender);
    if (hr != NOERROR)
    {
        ErrMsg(TEXT("Error VideoRenderer filter"));
        return FALSE;
    }

    hr = gcap.pFg->AddFilter(gcap.pRender, L"Video Renderer");
    if (hr != NOERROR)
    {
        ErrMsg(TEXT("Error %x: Cannot add Video Renderer to filtergraph"), hr);
        return FALSE;
    }

    // start connecting the filters
    // connect the video capture and mjpeg filters
    hr = gcap.pBuilder->ConnectFilters(gcap.pVCap, &PIN_CATEGORY_CAPTURE, &MEDIATYPE_Video, gcap.pMjpegDec);
    if (hr != NOERROR)
    {
        ErrMsg(TEXT("Error %x: Cannot connect video capture and mjpeg filters"), hr);
        return FALSE;
    }

    if (gcap.pSelectedFilter)
    {
        // connect the mjpeg and ezrgb filter
        hr = gcap.pBuilder->ConnectFilters(gcap.pMjpegDec, NULL, NULL, gcap.pSelectedFilter);
        if (hr != NOERROR)
        {
            ErrMsg(TEXT("Error %x: Cannot connect mjpeg and %s filters"), hr, psFilterName);
            return FALSE;
        }

        // connect the ezrgb and video renderer filters
        hr = gcap.pBuilder->ConnectFilters(gcap.pSelectedFilter, NULL, NULL, gcap.pRender);
        if (hr != NOERROR)
        {
            ErrMsg(TEXT("Error %x: Cannot connect %s and video renderer filters"), hr, psFilterName);
            return FALSE;
        }
    }
    else
    {
        // connect the mjpeg and video renderer filters
        hr = gcap.pBuilder->ConnectFilters(gcap.pMjpegDec, NULL, NULL, gcap.pRender);
        if (hr != NOERROR)
        {
            ErrMsg(TEXT("Error %x: Cannot connect mjpeg and video renderer filters"), hr);
            return FALSE;
        }
    }
...
{
    HRESULT hr;
    LPTSTR psFilterName = L"Print Analysis";

    if (gcap.pSelectedFilter == gcap.pEZrgb)
    {
        ErrMsg(TEXT("Error: Currently selected output from the capture device is YUV, EZrgb filter will not be used"));
    }

    if (gcap.pSelectedFilter && gcap.pSelectedFilter == gcap.pPrintAnalysis)
    {
        // add the video processing filter to the graph
        hr = gcap.pFg->AddFilter(gcap.pSelectedFilter, psFilterName);
        if (hr != NOERROR)
    ...
    if (gcap.pSelectedFilter && gcap.pSelectedFilter == gcap.pPrintAnalysis)
    {
        // connect the video capture and print analysis filters
        hr = gcap.pBuilder->ConnectFilters(gcap.pVCap, &PIN_CATEGORY_CAPTURE, &MEDIATYPE_Video, gcap.pSelectedFilter);
        if (hr != NOERROR)
        {
            ErrMsg(TEXT("Error %x: Cannot connect video capture and %s filters"), hr, psFilterName);
            return FALSE;
        }

        // connect the print analysis and video renderer filters
        hr = gcap.pBuilder->ConnectFilters(gcap.pSelectedFilter, NULL, NULL, gcap.pRender);
        if (hr != NOERROR)
        {
            ErrMsg(TEXT("Error %x: Cannot connect %s and video renderer filters"), hr, psFilterName);
    ...
    {
        // connect the capture and video renderer filters
        hr = gcap.pBuilder->ConnectFilters(gcap.pVCap, NULL, NULL, gcap.pRender);
        if (hr != NOERROR)
        {
            ErrMsg(TEXT("Error %x: Cannot connect capture and video renderer filters"), hr);
            return FALSE;
        }
    }
...
BOOL BuildPreviewGraph()
{
    ...
    if (gcap.pVSC)
    {
        ...
        if (hr == NOERROR)
        {
            ...
            {
                if (!BuildPreviewGraphRGB())
                    return false;
            }
            else if (pmt->subtype == MEDIASUBTYPE_YUY2)
            {
                if (!BuildPreviewGraphYUV())
                    return false;
            }
            else
            {
    ...
    SaveGraphFile(gcap.pFg, L"test.grf");
...
BOOL BuildPreviewGraph()
{
    ...
    if (gcap.pEZrgb)
    {
        hr = gcap.pFg->AddFilter(gcap.pEZrgb, L"EZ Rgb24");
        if (hr != NOERROR)
        {
            ErrMsg(TEXT("Error %x: Cannot add EZrgb24 to filtergraph"), hr);
            return FALSE;
        }
    }
    //
    ...
    {
        hr = gcap.pBuilder->RenderStream(&PIN_CATEGORY_PREVIEW,
                                         &MEDIATYPE_Interleaved, gcap.pVCap, gcap.pEZrgb, NULL);
        if(hr == VFW_S_NOPREVIEWPIN)
        {
            // preview was faked up for us using the (only) capture pin
            gcap.fPreviewFaked = TRUE;
        ...
        else if(hr != S_OK)
        {
            // maybe it's DV?
            hr = gcap.pBuilder->RenderStream(&PIN_CATEGORY_PREVIEW,
                                             &MEDIATYPE_Video, gcap.pVCap, gcap.pEZrgb, NULL);
            if(hr == VFW_S_NOPREVIEWPIN)
            {
                // preview was faked up for us using the (only) capture pin
                gcap.fPreviewFaked = TRUE;
...
void MakeMenuOptions()
{
    ...
    gcap.iEZrgbDialogsPos = -1;
    gcap.iPrintAnalysisDlgPos = -1;
    ...
    if (gcap.pEZrgb)
    {
        hr = gcap.pEZrgb->QueryInterface(IID_ISpecifyPropertyPages, (void**)&pSpec);
        if (SUCCEEDED(hr))
        {
            hr = pSpec->GetPages(&cauuid);
            if (SUCCEEDED(hr) && cauuid.cElems > 0)
            {
                AppendMenu(hMenuSub, MF_STRING, MENU_DIALOG0 + zz, TEXT("EZrgb24 Filter..."));
                gcap.iEZrgbDialogsPos = zz++;
                CoTaskMemFree(cauuid.pElems);
            }
            pSpec->Release();
        }
    ...
    if (gcap.pPrintAnalysis)
    {
        hr = gcap.pPrintAnalysis->QueryInterface(IID_ISpecifyPropertyPages, (void**)&pSpec);
        if (SUCCEEDED(hr))
        {
            hr = pSpec->GetPages(&cauuid);
            if (SUCCEEDED(hr) && cauuid.cElems > 0)
            {
                AppendMenu(hMenuSub, MF_STRING, MENU_DIALOG0 + zz, TEXT("Print Analysis Filter..."));
                gcap.iPrintAnalysisDlgPos = zz++;
                CoTaskMemFree(cauuid.pElems);
            }
            pSpec->Release();
        }
...
#define DESCSIZE 80
    ...
    {
        if(gcap.rgpmAudioMenu[i] == NULL || gcap.pmAudio == NULL)
            break;
...
void EnableFilterMenu(BOOL bEnable)
{
    HMENU hMenuSub = GetSubMenu(GetMenu(ghwndApp), 4);        // Devices menu

    if (bEnable)
    {
        EnableMenuItem(hMenuSub, MENU_EZRGB_FILTER, gcap.pEZrgb ? MF_ENABLED : MF_DISABLED);
        EnableMenuItem(hMenuSub, MENU_PA_FILTER, gcap.pPrintAnalysis ? MF_ENABLED : MF_DISABLED);
    }
    else
    {
        EnableMenuItem(hMenuSub, MENU_EZRGB_FILTER, MF_DISABLED);
        EnableMenuItem(hMenuSub, MENU_PA_FILTER, MF_DISABLED);
    }
}

void CheckFilterMenu()
{
    HMENU hMenuSub = GetSubMenu(GetMenu(ghwndApp), 4);        // Devices menu
    ...
    if (gcap.pSelectedFilter)
    {
        if (gcap.pSelectedFilter == gcap.pEZrgb)
        {
            CheckMenuItem(hMenuSub, MENU_EZRGB_FILTER, MF_CHECKED);
            CheckMenuItem(hMenuSub, MENU_PA_FILTER, MF_UNCHECKED);
        }
        else if (gcap.pSelectedFilter == gcap.pPrintAnalysis)
        {
            CheckMenuItem(hMenuSub, MENU_EZRGB_FILTER, MF_UNCHECKED);
            CheckMenuItem(hMenuSub, MENU_PA_FILTER, MF_CHECKED);
        }
    }
    else
    {
...
void AddFiltersToMenu()
{
    ...
    hMenuSub = GetSubMenu(GetMenu(ghwndApp), 4);        // Devices menu

    AppendMenu(hMenuSub, MF_STRING, MENU_EZRGB_FILTER, L"EZrgb Filter");
    AppendMenu(hMenuSub, MF_STRING, MENU_PA_FILTER, L"Print Analysis Filter");
...
LONG PASCAL AppCommand(HWND hwnd, unsigned msg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch(id)
    {
        ...
        case MENU_EZRGB_FILTER:
            if (gcap.pSelectedFilter == gcap.pEZrgb)
            {
                // already selected, we need to unselect it
                gcap.pSelectedFilter = NULL;
            }
            else
            {
        ...
        case MENU_PA_FILTER:
            if (gcap.pSelectedFilter == gcap.pPrintAnalysis)
            {
                // already selected, we need to unselect it
                gcap.pSelectedFilter = NULL;
            }
        ...
        case MENU_DIALOGF:
            ...
            else if (id - MENU_DIALOG0 == gcap.iEZrgbDialogsPos)
            {
                ShowPropertyPages(gcap.hEZrgbModule, CLSID_EZrgb24PropertyPage, gcap.pEZrgb);
            }
            else if (id - MENU_DIALOG0 == gcap.iPrintAnalysisDlgPos)
            {
                ShowPropertyPages(gcap.hPrintAnalysisModule, CLSID_PrintAnalysisPropertyPage, gcap.pPrintAnalysis);
            }
...
void OnClose()
{
    ...
    if (gcap.hEZrgbModule)
        FreeLibrary(gcap.hEZrgbModule);

    if (gcap.hPrintAnalysisModule)
        FreeLibrary(gcap.hPrintAnalysisModule);
