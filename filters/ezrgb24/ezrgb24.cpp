    ...
    CPersistStream(punk, phr)
{
    ...
    GetProfileStringA("Quartz", "EffectLength", "1000.0", sz, 60);
...
//#define max_hist_width (5000)
//int r[max_hist_width], g[max_hist_width], b[max_hist_width];

int Normalize(float value, float minVal, float maxVal, float rangeMin, float rangeMax)
{
    float ans = (((value - minVal) / (maxVal - minVal)) * (rangeMax - rangeMin)) + rangeMin;
    return ans;
}
...
HRESULT CEZrgb24::Transform(IMediaSample* pMediaSample)
{
    ...
    int hist_min_y, hist_max_y, hist_mid_y, hist_height = 100;
    int rmax, rmin, gmax, gmin, bmax, bmin;
    int minmin, maxmax, plot_min_y, range;
    unsigned char* pchn;  // pointer to color channel march from r to g to b to next r etc..
    int *r, *g, *b;
...
#define ROW(y) &pData[cxImage * 3 * y]

    hist_mid_y = cyImage / 2;  // half way
    hist_height = 64;
    hist_min_y = hist_mid_y - hist_height / 2;
    hist_max_y = hist_min_y + hist_height;

    // Draw the bottom line of our area of interest
    prgb = (RGBTRIPLE*)ROW(hist_min_y);

    for (x = 0; x < cxImage; x++) {
        prgb[x].rgbtRed = prgb[x].rgbtGreen = prgb[x].rgbtBlue = 0;
    }
    
    // Draw the top line of our area of interest
    prgb = (RGBTRIPLE*)ROW(hist_max_y);
    ...
    for (y = hist_min_y + 1; y < hist_max_y-1; y++)
    {
        prgb = (RGBTRIPLE*)ROW(y);

        for (x = 0; x < cxImage; x++) {
            r[x] += prgb[x].rgbtRed;
            g[x] += prgb[x].rgbtGreen;
            b[x] += prgb[x].rgbtBlue;
        }
    }
    ...
    rmax = gmax = bmax = 0;
    rmin = gmin = bmin = 255 * hist_height;

    for (x = 0; x < cxImage; x++) {
        if (r[x] > rmax) rmax = r[x];
        if (r[x] < rmin) rmin = r[x];
        if (g[x] > gmax) gmax = g[x];
        if (g[x] < gmin) gmin = g[x];
        if (b[x] > bmax) bmax = b[x];
        if (b[x] < bmin) bmin = b[x];
    }
    minmin = rmin;
    if (gmin < minmin) minmin = gmin;
    if (bmin < minmin) minmin = bmin;
    maxmax = rmax;
    if (gmax > maxmax) maxmax = gmax;
    if (bmax > maxmax) maxmax = bmax;
    range = maxmax - minmin;

    int maxRange = cyImage - 5;
    int minRange = 5;
    //int maxRange = hist_max_y;
    //int minRange = hist_min_y;

    for (x = 0; x < cxImage; x++) {
        y = Normalize(r[x], rmin, rmax, minRange, maxRange);
        prgb = (RGBTRIPLE*)ROW(y);
        prgb[x].rgbtRed = 255; prgb[x].rgbtGreen = 0; prgb[x].rgbtBlue = 0;

        y = Normalize(g[x], gmin, gmax, minRange, maxRange);
        prgb = (RGBTRIPLE*)ROW(y);
        prgb[x].rgbtRed = 0; prgb[x].rgbtGreen = 255; prgb[x].rgbtBlue = 0;

        y = Normalize(b[x], bmin, bmax, minRange, maxRange);
        prgb = (RGBTRIPLE*)ROW(y);
        prgb[x].rgbtRed = 0; prgb[x].rgbtGreen = 0; prgb[x].rgbtBlue = 255;
    }
    ...
    case IDC_RED:
        ...
        for (iPixel = 0; iPixel < numPixels / 2; iPixel++, prgb++) {
            prgb->rgbtGreen = 0;
            prgb->rgbtBlue = 0;
        }
        ...
        for (x = 1; x < 100; ++x) {
            //              for (y = 1; y < 400; ++y) {
            y = 3 * x;
            ptry = (BYTE*)&pData[y * cxImage * 3 + x * 3];
            prgb = (RGBTRIPLE*)ptry;
            prgb->rgbtBlue = 0x00;// prgb++;
            prgb->rgbtRed = 0x00;// prgb++;
            prgb->rgbtGreen = 0xff;// prgb++;
//   prgb->rgbtBlue = 250; prgb++;
                //    prgb->rgbtBlue = 250;
  //              }
        }
        break;
    ...
    case IDC_EMBOSS:
        prgb = (RGBTRIPLE*)pData;
        int half = pvi->bmiHeader.biHeight >> 1;
        for (y = 0; y < pvi->bmiHeader.biHeight; y++)
        {
            ...
            for (x = 1; x < pvi->bmiHeader.biWidth; x++) {
                ...
                if (grey2 > 0) {
                    ...
                    prgb->rgbtRed = prgb->rgbtGreen = prgb->rgbtBlue = (BYTE)0;
                }
                prgb++;
            }
        }
